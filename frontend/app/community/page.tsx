...
export default function CommunityPage() {
  ...
  return (
    <>
      <Header />
      <main className="flex-1 bg-background text-foreground flex flex-col min-h-[calc(100vh-3.5rem)]">
        <div className="mx-auto w-full max-w-3xl px-4 py-6 flex-1 flex flex-col">
          <div className="flex items-center justify-between border-b border-border pb-4 mb-4">
            <div>
              ...
              <p className="text-xs text-secondary mt-1">
                A live space for the community. No profiles. No public identities.
              </p>
            </div>
          ...
          <div className="flex-1 border border-border bg-surface rounded-xl flex flex-col overflow-hidden min-h-[350px] relative">
            <div
              ref={scrollRef}
              className="flex-1 overflow-y-auto p-4 sm:p-5 flex flex-col gap-4 scroll-smooth"
            >
              {messages.length > 0 ? (
                messages.map((msg, idx) => (
                  <div key={msg.id} className="flex flex-col gap-1">
                    {idx > 0 && <div className="border-t border-border/40 my-3" />}
                    <div className="flex flex-col gap-1">
                      <p className="text-sm leading-relaxed text-secondary whitespace-pre-line break-words">
                        {msg.content}
                      </p>
                      <div className="flex justify-end text-[10px] text-muted font-medium mt-0.5">
            ...
            {unreadCount > 0 && (
              <button
                ...
                className="absolute bottom-20 left-1/2 -translate-x-1/2 bg-accent text-background text-xs font-bold px-3 py-1.5 rounded-full shadow-lg border border-accent hover:opacity-90 transition-all flex items-center gap-1.5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-focus focus-visible:ring-offset-2"
              >
